...
# =================================================================================
##################################################################################
def main_loop(batch_size=config.batch_size, model_type='', tensorboard=True, train_ratio=1.0):
    ...
    if config.task_name == 'MoNuSeg':
        ...
        train_dataset = ImageToImage2D(config.train_dataset, config.task_name, train_text, train_tf,
                                       image_size=config.img_size)
        val_dataset = ImageToImage2D(config.val_dataset, config.task_name, val_text, val_tf, image_size=config.img_size)
    elif config.task_name == 'Kvasir_SEG':
        train_text = read_text(config.train_dataset + 'Train_text.xlsx')
        val_text = read_text(config.val_dataset + 'Val_text.xlsx')
        train_dataset = ImageToImage2D(config.train_dataset, config.task_name, train_text, train_tf,
                                    image_size=config.img_size)
    ...
    if train_ratio < 1.0:
        ...
        subset_size = max(1, int(num_train * train_ratio))
        indices = random.sample(range(num_train), subset_size)
        train_dataset = Subset(train_dataset, indices)
        logger.info('Using {:.0%} of training data: {}/{} samples'.format(train_ratio, subset_size, num_train))
    ...
    elif model_type == 'MedMARS_pretrain':
        config_vit = config.get_CTranS_config()
        logger.info('transformer head num: {}'.format(config_vit.transformer.num_heads))
        logger.info('transformer layers num: {}'.format(config_vit.transformer.num_layers))
        logger.info('transformer expand ratio: {}'.format(config_vit.expand_ratio))
        model = MedMARS(config_vit, n_channels=config.n_channels, n_classes=config.n_labels, img_size=config.img_size)
        pretrained_UNet_model_path = ""
    ...
    logger.info(contrastive_weight)
    criterion = MultiTaskLoss(seg_weight=1.0, contrastive_weight=contrastive_weight)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_ratio', type=float, default=1.0,
                        help='Fraction of training data to use (e.g. 0.5, 0.75, 1.0)')
    args = parser.parse_args()

    assert 0 < args.train_ratio <= 1.0, 'train_ratio must be in (0, 1]'
    ...
    model = main_loop(model_type=config.model_name, tensorboard=True, train_ratio=args.train_ratio)
