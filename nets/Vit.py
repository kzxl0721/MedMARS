...
class VisionTransformer(nn.Module):
    def __init__(self, config, vis, img_size, channel_num, patch_size, embed_dim, depth=1, num_heads=8,
        ...
        self.CTBN3 = ConvTransBN(in_channels=embed_dim, out_channels=self.num_patches)
        self.text_projection = nn.Linear(self.num_patches, embed_dim)  # [B, 10, 1024] -> [B, 10, 64]
        self.final_projection = nn.Linear(77, self.num_patches)  # [B, 64, 10] -> [B, 64, 1024]

    def forward(self, x, skip_x, text, reconstruct=False):
        if not reconstruct:
            x = self.embeddings(x)  # [B, 1024, 64]
            text_out = self.CTBN3(text.transpose(1, 2)).transpose(1, 2)  # [B, 10, 1024]
            text_out = self.text_projection(text_out)  # [B, 10, 64]
            text_out = text_out.transpose(1, 2)  # [B, 64, 10]
            text_out = self.final_projection(text_out)  # [B, 64, 1024]
            text_out = text_out.transpose(1, 2)  # [B, 1024, 64]
            if self.dim == 64:
                x = x + text_out  # [B, 1024, 64]
